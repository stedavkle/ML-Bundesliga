...
def test_get_matches_from_league_and_season_from_API(league, season):
    ...
    assert (test_matches.matchday <= 40).all()
...
#test_get_matches_from_league_and_season_from_API(3, 2008)
...
def test_get_next_matchday():
    matchday = cr.get_next_matchday()
    assert isinstance(matchday, dict)
    assert len(matchday) == 3
...
def test_get_teams(league, season):
...
@pytest.mark.parametrize('league', [1,2,3])
@pytest.mark.parametrize('season', [2020,2015,2010])
def test_load_season(league, season):
    matches, results, loaded = cr.load_season(league, season)
    # TEST MATCHES
...
def test_create_dataset_recursive(leagues, seasons, day_start, day_end):
...
@pytest.mark.parametrize('leagues', [[1], [2], [3]])
@pytest.mark.parametrize('seasons', [[2020], [2015], [2010]])
def test_get_team_dicts(leagues, seasons):
    id_to_team, team_to_id = cr.get_team_dicts(leagues, seasons)
...
@pytest.mark.parametrize('seasons', [[2020, 2015, 2010]])
@pytest.mark.parametrize('leagues', [[1, 2, 3]])
def test_get_data_for_algo(leagues, seasons):
